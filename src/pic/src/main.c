...
#define INIT_TIMER0 0
#define INIT_TIMER1 1
...
void main(void) {
    ...
    uart_comm uc;
    uart_comm uc_tx;
    unsigned char uartbuffer[MAXUARTBUF + 1];
...
#endif
    adc_thread_struct adcthread_data; // Used to keep persistent A/D readings
    ...
    // initialize my uart handler if supported
#ifndef __USE18F26J50
    init_uart(&uc, &uc_tx);
#endif
    ...
    TRISB = 0x03; // Set int1/int0 to input
    //LATB = 0x3F; // Previously set to 0x0
    INIT_RED_LEDS();

    TRISC = 0x00; // Previously not set
    //LATC = 0x3F; // Previously not set
    INIT_YELLOW_LEDS();
#endif
...
#else
#if INIT_TIMER1 && !MOTOR_PIC || MOTOR_PIC
...
#if 1
#if I2C_MASTER
    i2c_configure_master(0x4E);
#elif SENSOR_PIC
    i2c_configure_slave(0x4E);
#elif MOTOR_PIC
    initEncoders();
    i2c_configure_slave(0x1E);
    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                case MSGT_I2C_DBG:
                {
                    ...
                    // Special case for motor handler
                    switch (last_reg_recvd) {
                        case MSGT_SET_MOTOR_DATA: {
                            setMotorSpeeds(msgbuffer[1], msgbuffer[2]);
                            break;
                        };
                        default: { // No special write case
                            break;
                        };
                    };
                ...
                case MSGT_ADC_RECV_COMPLETE: // Handles ADC code
                {
                    adc_lthread(&adcthread_data, msgtype, length, msgbuffer);
                    break;
                };
                case MSGT_I2C_TO_UART: // Sends I2C as UART
                {
                    // Shift all message bytes over one to the right
                    {
                        char i;
                        for (i = length; i > 0; --i) {
                    ...
                    // There is now room for a byte in front
                    length++;

                    // Insert header byte into front
                    msgbuffer[0] = HEADER_DATA_BYTE;

                    // Add checksum byte into back
                    msgbuffer[length] = constructChecksum(msgbuffer, length);

                    // There is now an checksum byte added
                    length++;

                    //DBG1_TOGGLE();

                    // Send message back to ARM
                ...
                case MSGT_I2C_RQST:
                {
                    ...
                    switch (last_reg_recvd) {
                        // Senkbeil: Should look into custom request (high-priority)
                        //           from ARM that asks for data, would cause an
                        //           "interrupt" that would access the ADC struct
                        //           and get the oldest data to send
                        //
                        //           Note: If stack of old values actually happens,
                        //                 would cause lag on the ARM; so, may
                        //                 have to simply throw out old value
                        //                 for a new one.

                        case MSGT_GET_ENCODER_DATA: // Get encoder data
                        {
#if MOTOR_PIC
                            A0_TOGGLE();

                            length = 5;
                            msgbuffer[0] = 0xC1; //MSGT_RETURN_ENCODER_DATA;

                            // Dummy data
                            left = getLeftChange();
                            right = getRightChange();
                            msgbuffer[2] = left &  0x00FF;
                            msgbuffer[1] = (left &  0xFF00)>>8;
                            msgbuffer[4] = right & 0x00FF;
                            msgbuffer[3] = (right & 0xFF00)>>8;
#endif
                            break;
                        }
                        case MSGT_SET_MOTOR_DATA: // Set motor data (response indicates success)
                        {
                            A1_TOGGLE();

                            length = 1;
                            msgbuffer[0] = 0xC2; //MSGT_RETURN_MOTOR_DATA;
                            break;
                        }
                        case MSGT_GET_SENSOR_DATA: // Get sensor data
                        {
                            A2_TOGGLE();

                            length = 5;
                            msgbuffer[0] = 0xD1; //MSGT_RETURN_SENSOR_DATA;

                            // ADC Data
                            msgbuffer[1] = (adcthread_data.value[0] & 0x03FC) >> 2;
                            msgbuffer[2] = (adcthread_data.value[1] & 0x03FC) >> 2;
                            msgbuffer[3] = (adcthread_data.value[2] & 0x03FC) >> 2;
                            msgbuffer[4] = (adcthread_data.value[3] & 0x03FC) >> 2;
                            break;
                        }
                    };
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                ////////////////////////////////////////////////////////////////
                // EMERGENCY CHECK FOR PIC STATES
                ////////////////////////////////////////////////////////////////
                case MSGT_TIMER1:
                {
                    // Feed into the timer thread the current state of the slave
                    // so it can be corrected if necessary
#if !I2C_MASTER
                    length = 1;
                    timer1_lthread(&t1thread_data, msgtype, length, &last_reg_recvd);
#else
...
#ifndef __USE18F26J50
                ////////////////////////////////////////////////////////////////
                // RECEIVED VALID MESSAGE VIA UART
                ////////////////////////////////////////////////////////////////
                case MSGT_UART_DATA:
                {
                    t1thread_data.msgcount = 0; // Reset panic
                    uart_lthread(&uthread_data, msgtype, length, msgbuffer);
                    break;
                };
                case MSGT_UART_TO_I2C: // Sends UART as I2C message
                { A0_TOGGLE();
#if I2C_MASTER
                    // Handle cases in specific manners
                    switch (msgbuffer[0]) {
                        case MSGT_SET_MOTOR_DATA: {
                            if (i2c_master_send(length, msgbuffer) != 0);
                            if (i2c_master_recv(1) != 0);
                            break;
                        };
